...
def _isles_of_sea_and_sky_has_plot(state: CollectionState, player: int, item: str):
...
def _isles_of_sea_and_sky_can_level(exp: int, lvl: int):
    if exp >= 10 and lvl == 1:
        return True
...
def set_rules(world: "IslesOfSeaAndSkyWorld"):
    ...
    set_rule(multiworld.get_entrance("Core Exit", player),
             lambda state: _isles_of_sea_and_sky_has_plot(state, player, "Mettaton Plush"))
    ...
    if _isles_of_sea_and_sky_is_route(world, 1):
        set_rule(multiworld.get_entrance("Papyrus\" Home Entrance", player),
                 lambda state: _isles_of_sea_and_sky_has_plot(state, player, "Complete Skeleton"))
        set_rule(multiworld.get_entrance("Undyne\"s Home Entrance", player),
                 lambda state: _isles_of_sea_and_sky_has_plot(state, player, "Fish") and state.has("Papyrus Date", player))
    ...
    if (not _isles_of_sea_and_sky_is_route(world, 2)) or _isles_of_sea_and_sky_is_route(world, 3):
        ...
        set_rule(multiworld.get_location("Apron Hidden", player),
                 lambda state: state.can_reach("Cooking Show", "Region", player))
    if _isles_of_sea_and_sky_is_route(world, 2) and \
        ...
        maxlv = 1
        while maxlv < 20:
            while _isles_of_sea_and_sky_can_level(exp, maxlv):
                maxlv += 1
                if world.options.rando_stats:
                    if curarea == "Old Home":
